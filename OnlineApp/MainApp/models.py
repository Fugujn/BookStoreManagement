...
class User(db.Model, UserMixin):
    ...
    name = db.Column(db.String(64), nullable=False, default=email)
    image_file = db.Column(db.String(255), nullable=False,
                           default='https://res.cloudinary.com/ddgtjayoj/image/upload/v1703609574/nwgptms1x2n1pa4k5hvo.jpg')
...
class Book(db.Model):
    ...
    image_src = Column(LONGTEXT, nullable=True,
                       default='https://res.cloudinary.com/ddgtjayoj/image/upload/v1703609574/nwgptms1x2n1pa4k5hvo.jpg')
...
class Order(db.Model):
    ...
    def order_total(self):
        return db.session.quey(db.func.sum(OrderDetails.quantity * Book.unit_price)).join(Book).filter(
            OrderDetails.order_id == self.id).scalar() + 1000
