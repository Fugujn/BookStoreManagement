...
class UserAdmin(MyModelView):
    ...
    column_labels = {'name': 'Name', 'email': 'Email Address', 'roles': 'Role'}
    column_filters = ('name', 'email', 'roles.name')
        ...
        if form.validate():
            try:
                if int(form.min_import_quantity.data) > 0 and int(form.min_stock_quantity.data) > 0 and int(
                    ...
                    model.time_to_end_register = form.time_to_end_order.data
                    db.session.commit()
                    return True
                else:
